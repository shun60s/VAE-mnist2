...
"""
 Change from torchvison mnist.py  is to make a dataset that includes specified digit number only in class MNIST().
 Please see LICENSE-torchvision about torchvison. 
 指定された数字だけのデータセットを作成する
 
"""

from torchvision.datasets.vision import VisionDataset  ### change
import warnings
...
class MNIST(VisionDataset):
    ...
    def __init__(
            ...
            download: bool = False,
            digit_number: int = 1,   ### add  specify digit number.
    ) -> None:
        super(MNIST, self).__init__(root, transform=transform,
                                    target_transform=target_transform)
        
        ### add
        assert digit_number >= 0 and digit_number <= 9  # digit_number is digit number 0-9.
        self.digit_number = digit_number
        self.training_file= "training" + str(self.digit_number) +'.pt'
        self.test_file= "test" + str(self.digit_number) +'.pt'
        ### end of add
    ...
    def download(self) -> None:
        ...
        train_image = read_image_file(os.path.join(self.raw_folder, 'train-images-idx3-ubyte'))
        
        test_label  = read_label_file(os.path.join(self.raw_folder, 't10k-labels-idx1-ubyte'))
        test_image  = read_image_file(os.path.join(self.raw_folder, 't10k-images-idx3-ubyte'))
        
        """
        print (train_label[ train_label.eq(self.digit_number) ])
        print ( train_image.size())
        print ( train_image[ train_label.eq(self.digit_number)].size() )
        print ( test_label[ test_label.eq(self.digit_number) ])
        print ( test_image.size())
        print ( test_image[ test_label.eq(self.digit_number)].size() )
        os.exit()
        """
        ### change
        training_set = (
                          train_image[ train_label.eq(self.digit_number)],
                          train_label[ train_label.eq(self.digit_number)]
                       )
        
        test_set = (
                          test_image[ test_label.eq(self.digit_number)],
                          test_label[ test_label.eq(self.digit_number)]
                       )
